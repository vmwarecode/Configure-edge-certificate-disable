...
def api_save_certificate(edge_id,count,edge_name):
    ...
    data = (
        ...
        '"params":{"id":%s,"_update":{"endpointPkiMode":"CERTIFICATE_DISABLED"}},"id":%s}'
        % (edge_id,count)
    )
    resp = requests.post(vco_url, headers=headers, data=data)
    if resp.status_code == 200:
        logger.warning(f'finished setting certificate disabled for edge {edge_name}')
        return True
    else:
        logger.warning(f'failed setting certificate disabled for edge {edge_name} reason {str(resp.status_code)} , {resp.reason}')
        return False
...
def validate_certifcate():
    ...
    for edge in edges:
        if edge["endpointPkiMode"] != "CERTIFICATE_OPTIONAL":
            c+=1
    logger.warning(f'pending certificate disable total = {c}')
